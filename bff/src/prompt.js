...
## X（旧Twitter）の紹介文
- xPost に、このレシピを X に投稿するための短い紹介文を書く。
- **125文字以内。** 投稿時に URL を足すので、それより長くしない。
- **ハッシュタグは書かない。** 1つも付けない。
- URL は書かない。投稿するときにこちらで付ける。

### 組み立て
食材の扱い方を示してから、その例としてこの料理を出す。

1. **書き出しは食材の方針。** その食材をどう扱うのが基本かを言い切る。
   例:「豚ひき肉は炒め料理を基本に考える。」「ごぼうは煮物と相性がいい。」
   **例文の言い回しをそのまま使わない。** 食材ごとに素直な言い方を選ぶ。
   とくに「〜と相性がいい」を毎回使わない。
2. **次にこの料理を名指しする。** 例:「特におすすめのメニューはドライカレー。」
3. **作り方を短い文に分けて書く。** 手順そのままではなく、要点だけ。
   分量の比率や温度など、押さえどころの数字は入れる。
4. **締めはなぜそうなるかの原理。** 例:「ごぼうは甘味と旨みの強さを合わせると香りの強さが引き立つ。」
   **確かなことが書けるときだけ書く。** 思いつかなければ 3 で終える。
   それらしいだけの説明を作らない。
...
- note「自炊ハック」と同じ調子にする。
  ...
  - 比喩・ジョーク・脱線を入れない。
- **一文を短く切る。** 情報を1つの文に詰め込まない。
  「玉ねぎと揚げなすを加えてカレー粉で炒め、ルーは1/3で仕上げる15分カレー。」ではなく、
  「玉ねぎと揚げなすを加えて。カレー粉で炒め、ルーは1/3で仕上げる。」
- **料理名を体言止めで文末に置かない。**「〜で仕上げる15分カレー。」のような書き方をしない。
  料理名は 2 で名指しする。
- **飾った動詞を使わない。**「締める」ではなく「味を整える」。「香る」ではなく「合う」。
- 「作り置きおかず」「時短メニュー」のような分類語でまとめない。何ができるかを直接書く。
- 数字をでっち上げない。**所要時間は無理に入れなくてよい。** 分からなければ触れない。
    ...
    input_schema: {
      ...
      properties: {
        ...
        xPost: {
          ...
          maxLength: 125,
          description:
            "X に投稿する紹介文。125文字以内。導入なし・一人称なし・数字を入れる。" +
            "食材の扱い方の方針で始め、料理名を名指しし、作り方を短い文で書く。" +
            "確かなことが書けるなら原理で締める。一文を短く切る。" +
            "ハッシュタグと URL は書かない。",
        },
        imageKind: {
          type: "string",
