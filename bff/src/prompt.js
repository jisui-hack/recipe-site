...
- **120文字以内。** 投稿時に URL を足すので、それより長くしない。
- note「自炊ハック」と同じ調子にする。
  - 導入を書かない。いきなり本題から始める。
  - 一人称（私・僕）を使わない。体験談・感想を書かない。
  - 比喩・ジョーク・脱線を入れない。
- 何がどれくらいの手間でできるかを、具体的な数字で書く（所要時間・人数）。
- 末尾にハッシュタグを2〜4個。**#自炊ハック は必ず入れる。**
- URL は書かない。投稿するときにこちらで付ける。
- 所要時間が分からない場合は時間に触れない。数字をでっち上げない。
    ...
    input_schema: {
      ...
      properties: {
        ...
        xPost: {
          ...
          maxLength: 120,
          description:
            "X に投稿する紹介文。120文字以内。導入なし・一人称なし・数字を入れる。" +
            "末尾にハッシュタグ2〜4個（#自炊ハック は必須）。URL は書かない。",
        },
        imageKind: {
          type: "string",
          enum: [...IMAGE_KINDS],
