...
`predeploy` が先に走り、次のどれかに当たると止まる。

| 止まる条件 | なぜ |
|---|---|
| main 以外にいる | 機能ブランチの中身をそのまま公開してしまう |
| `origin/main` より遅れている | **他の変更を巻き戻す** |
| コミットしていない変更がある | 手元だけの状態が公開される |

> **実際にこれで事故った。** PR を出したあと、別作業のために main へ切り替えて
> `wrangler deploy` したところ、まだ main に入っていなかったプロンプトの変更が
> 本番から消え、X の投稿文が古い形式に戻った。
> **テストが通ることと、公開してよい中身であることは別。**

分かったうえで出すなら `DEPLOY_ANYWAY=1 npm run deploy`。
...
`wrangler.toml` の `X_POST_ENABLED = "false"`。**2026-09-17 に切った。**
レシピの蓄積に集中する期間、X 用の文章を作らない。

切ると変わるもの:

| | 作る | 切る |
|---|---|---|
| システムプロンプト | X の節を含む（約2,300字） | X の節なし（約1,200字） |
| ツール定義 | `xPost` あり | **`xPost` を定義から外す** |
| 応答 | `xPost` に本文 | `xPost` は空文字 |
| フロント | 「X に投稿する文」が出る | 出ない（空なら隠す作り） |

定義から外すのは、残すとモデルが required でなくても埋めにくるため。
出力トークンが減らない。

**戻し方**: `X_POST_ENABLED = "true"` にして `npm run deploy`。
コードは消していないので、プロンプト・定義・整形（タイトルを先頭に付ける、
ハッシュタグを落とす）がそのまま元に戻る。キャッシュ鍵にも含めているので、
切り替え直後に古い応答が混ざることはない。
